...
#		Formatting
#
#	Version 1.4.4, 18-Jan-2021, Dan K. Snelson
#		Added "--auto" option to auto-anwer "yes" to yes / no prompts
#		Added "--filter" option to filter results
...
function displayHelp() {
	...
	echo "
Jamf Pro Policy Editor Lite, ${scriptVersion}
...
function promptAPIurl() {
	...
	if [[ -z "${apiURL}" ]]; then
		...
		if [[ -e "/Library/Preferences/com.jamfsoftware.jamf.plist" ]]; then # read the API URL from the JAMF preferences
			...
			if [[ ${autoAnswer} == "yes" ]]; then
				...
				ScriptLog "Auto-answer equals \"yes\"; proceeding without prompting …"
				printf "\n\n*** Auto-answer equals \"yes\"; proceeding without prompting … ***"
				apiURL="${apiURL}"
...
function promptNewVersion() {
	...
	currentVersion=$( echo ${policyName} | egrep -o "([0-9]{1,}\.)+[0-9]{1,}" | xargs )	# Thanks, @dev-yeet
	...
	if [[ -z ${newVersion} ]]; then
		...
		printf "• Current version: ${currentVersion}\n"
		ScriptLog "New version number: ${newVersion}"
		printf "• New version number: ${newVersion}\n\n"
...
function promptToContinue(){
	...
	if [[ ${autoAnswer} == "yes" ]]; then
		...
		printf "*** Auto-answer equals \"yes\"; proceeding without prompting … ***"
...
do
	case "$1" in
		-f|--filter )
			shift
			policyNameSearchString="$1"
			;;
		-a|--auto ) autoAnswer="yes"
			;;
		-v|--version )
			shift
			newVersion="$1"
